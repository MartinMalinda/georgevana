...
namespace Pokemon
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Console.Write("I choose you, " + EffectiveCounter(pokemonOfAsh, wildPokemon).Name);
        }
        ...
        /*private static Pokemon EffectiveCounter(List<Pokemon> pokemonList, Pokemon rival)
        {
            foreach (var pokemon in pokemonList)
            {
                if (pokemon.IsEffectiveAgainst(rival))
                {
                    return pokemon;
                }
            }
            return pokemonList[0]; // assume there is no empty list coming in
        }*/
    }
}